...
let RoomDocumentService = class RoomDocumentService {
    /**
     * Class constructor
     *
    ...
    create(room) {
        return fromPromise_1.fromPromise(this._document.findOne({
            ...
            throw_1._throw(new Error(`Room with name '${room.name}' already exists`)) :
            fromPromise_1.fromPromise(this._document.create(room))), operators_1.map((doc) => doc.toJSON()));
    }
    /**
    ...
    findByIdAndUpdate(id, room) {
        return fromPromise_1.fromPromise(this._document.findByIdAndUpdate(id, room, { new: true }))
            .pipe(operators_1.flatMap((doc) => !!doc ?
            of_1.of(doc.toJSON()) :
            of_1.of(undefined)));
    }
...
RoomDocumentService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [mongo_1.MongoClientService])
], RoomDocumentService);
exports.RoomDocumentService = RoomDocumentService;
//# sourceMappingURL=room-document.service.js.map